...
- ✅ CI Guidelines Enforcement; Automatisierte Durchsetzung der Architektur- und Security-Konventionen: golangci-lint (5 neue Linter, 63 suppressed Findings), Layer-Violation Ratchet (37 Baseline-Violations), Svelte 4 Import Guard, Security Pattern Check (localStorage Auth-Token), 3 pre-commit Hooks, 2 CI Jobs, erweiterte Makefile-Targets; Refs: .golangci.yml, scripts/check-layer-violations.sh, scripts/check-svelte4-imports.sh, scripts/check-security-patterns.sh, lefthook.yml, .github/workflows/quality.yml, Makefile
