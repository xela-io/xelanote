<script lang="ts">
  ...
  interface Props {
    noteId: string;
    originalText: string; // Text that was formatted
    selectionFrom: number; // Start position in editor
    selectionTo: number; // End position in editor
    isFullContent: boolean; // true if entire note, false if selection
    initialContentHash: string; // Hash when dialog opened for conflict detection
    getCurrentContent: () => string; // Function to get current editor content
  ...
  const diffResult = $derived.by(() => {
    if (loading || error || !formattedText) return null;
    return diffLines(originalText, formattedText);
  });
  ...
  // Load formatted content on mount
  $effect(() => {
    formatContent();
  });
  ...
  async function formatContent() {
    ...
    try {
      const result = await api.formatMarkdown(noteId, originalText);
      ...
      if (e instanceof api.ApiError) {
        ...
        switch (e.status) {
          case 412: // Precondition Failed - no provider
            error = $_('error.format_markdown.no_provider');
            break;
          case 403: // Forbidden - AI disabled
            error = $_('error.format_markdown.ai_disabled');
            break;
          case 413: // Too large
            error = $_('error.format_markdown.too_large');
            break;
          case 400: // Too short or empty
            error = e.message.includes('short') ? $_('error.format_markdown.too_short') : e.message;
            break;
          case 504: // Gateway timeout
            error = $_('error.format_markdown.timeout');
            break;
          default:
            error = e.message;
        }
  ...
  async function handleApply() {
    if (loading || noChanges || !formattedText) return;
...
<div
  ...
  role="dialog"
  aria-modal="true"
  aria-labelledby="format-markdown-title"
  tabindex="-1"
  onkeydown={handleKeydown}
>
  <div
  ...
  >
    ...
    <div class="flex items-center justify-between p-4 border-b border-border flex-shrink-0">
      <h2 id="format-markdown-title" class="text-lg font-semibold flex items-center gap-2">
        <Wand2 size={20} />
        {$_('dialog.format_markdown.title')}
      </h2>
      <button
        type="button"
        onclick={onClose}
    ...
    <div class="flex-1 overflow-auto p-4">
      {#if loading}
        <div class="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <span class="text-muted-foreground">{$_('dialog.format_markdown.loading')}</span>
        </div>
      {:else if error}
        <div class="flex flex-col items-center justify-center py-12 gap-4">
      ...
      {:else if noChanges}
        <div class="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <span class="text-muted-foreground">{$_('dialog.format_markdown.no_changes')}</span>
        </div>
      {:else if diffResult}
        <!-- Diff view -->
    ...
    <div class="flex items-center justify-between p-4 border-t border-border flex-shrink-0 gap-4">
      <div class="text-xs text-muted-foreground">
        {#if !loading && !error && !noChanges}
          {isFullContent
            ? $_('dialog.format_markdown.full_note')
            : $_('dialog.format_markdown.selection_only')}
        {/if}
      </div>
      <div class="flex items-center gap-2">
        ...
        >
          {$_('dialog.format_markdown.apply')}
        </button>
      </div>
    </div>
  </div>
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
          <h3 id="conflict-title" class="font-semibold mb-2">
            {$_('dialog.format_markdown.conflict_title')}
          </h3>
          <p class="text-sm text-muted-foreground mb-4">
            {$_('dialog.format_markdown.conflict_warning')}
          </p>
          <div class="flex justify-end gap-2">
            ...
            >
              {$_('dialog.format_markdown.apply_anyway')}
            </button>
          </div>
        </div>
      </div>
