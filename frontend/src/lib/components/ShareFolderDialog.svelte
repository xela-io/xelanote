<script lang="ts">
  ...
  import type { FolderShare, UserSearchResult } from '$lib/api';
  ...
  const { folderId, encryptionDefault = false, onClose }: Props = $props();

  let searchQuery = $state('');
  let searchResults = $state<UserSearchResult[]>([]);
  let searching = $state(false);
  let shares = $state<FolderShare[]>([]);
  ...
  async function loadShares() {
    try {
      loadingShares = true;
      shares = await api.getFolderShares(folderId);
    } catch (err) {
  ...
  async function handleShare(user: UserSearchResult) {
    sharing = true;
    try {
      await api.shareFolder(folderId, user.username, selectedRole);
      toast.success($_('sharing.folder_share_success'));
      searchQuery = '';
      searchResults = [];
      await loadShares();
    } catch (err: unknown) {
      ...
      if (error.message?.includes('yourself')) {
        ...
        toast.error($_('sharing.folder_encrypted_warning'));
      } else {
  ...
  async function handleRemoveShare(userId: number) {
    try {
      await api.removeFolderShare(folderId, userId);
      toast.success($_('sharing.folder_unshare_success'));
      await loadShares();
    } catch (err: unknown) {
  ...
  async function handleRoleChange(userId: number, newRole: string) {
    try {
      await api.updateFolderShareRole(folderId, userId, newRole);
...
<BaseDialog open={true} title={$_('sharing.folder_title')} {onClose} size="md" scrollable>
  {#snippet content()}
    {#if encryptionDefault}
      <div
        class="bg-amber-500/10 border border-amber-500/30 rounded-md p-3 mb-4 text-sm text-amber-600 dark:text-amber-400"
      >
        {$_('sharing.folder_encrypted_warning')}
      </div>
    {:else}
